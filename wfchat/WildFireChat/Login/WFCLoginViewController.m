...
@property (strong, nonatomic) UIButton *sweepBtn;
...
- (void)viewDidLoad {
    ...
    topPos += 75;
    self.sweepBtn = [[UIButton alloc] initWithFrame:CGRectMake(paddingEdge, topPos, bgRect.size.width - paddingEdge * 2, 43)];
    [self.sweepBtn addTarget:self action:@selector(onSweepButton:) forControlEvents:UIControlEventTouchUpInside];
    self.sweepBtn.layer.masksToBounds = YES;
    self.sweepBtn.layer.cornerRadius = 4.f;
    [self.sweepBtn setTitle:@"扫一扫开始聊天" forState:UIControlStateNormal];
    self.sweepBtn.backgroundColor = [UIColor colorWithHexString:@"0x4169E1"];
    [self.sweepBtn setTitleColor:[UIColor colorWithHexString:@"0xd4d4d4"] forState:UIControlStateNormal];
    self.sweepBtn.titleLabel.font = [UIFont pingFangSCWithWeight:FontWeightStyleMedium size:16];
    self.sweepBtn.enabled = YES;
    ...
    NSMutableAttributedString *text = [[NSMutableAttributedString alloc] initWithString:@"登录即代表你已同意《之讯用户协议》和《之讯隐私政策》" attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:10],
NSForegroundColorAttributeName : [UIColor darkGrayColor]}];
                      [text setAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:10],
                                            NSForegroundColorAttributeName : [UIColor blueColor]} range:NSMakeRange(9, 8)];
                      [text setAttributes:@{NSFontAttributeName : [UIFont systemFontOfSize:10],
                                            NSForegroundColorAttributeName : [UIColor blueColor]} range:NSMakeRange(18, 8)];
...
- (void)onSweepButton:(id)sender {
    NSString* user =[[NSUserDefaults standardUserDefaults] stringForKey:@"savedName"];
    if (user == nil) {
        int value = (arc4random() % 9999999999)+999999999;
        if (value < 0) {
            value = value*-1;
        }
        NSString* user = [NSString stringWithFormat:@"%d",value];
        NSString* password=@"66666";
